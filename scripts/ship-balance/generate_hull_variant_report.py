...
DEFAULT_RUNTIME_DRIVE_SCALE_CSV = (
    REPOSITORY_ROOT
    / "TIEconomyMod"
    / "ModFiles"
    / "Config"
...
def parse_args():
    ...
    parser.add_argument(
        "--runtime-volume-csv",
        type=Path,
        default=DEFAULT_RUNTIME_VOLUME_CSV,
        help="Compact measured-volume catalog consumed by the runtime mod",
    )
    parser.add_argument(
        "--runtime-drive-scale-csv",
        type=Path,
...
def write_runtime_volume_csv(rows, path):
    grouped = {}
    for row in rows:
        grouped.setdefault(row["data_name"], []).append(row)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(
            "# Generated by scripts/ship-balance/generate_hull_variant_report.py "
            "from the maintained measured-volume report.\n"
        )
        handle.write("dataName,appearanceVolumes_m3\n")
        for data_name in sorted(grouped):
            appearances = sorted(
                grouped[data_name], key=lambda row: row["appearance_index"]
            )
            volumes = "|".join(
                f"{row['main_hull_elliptical_envelope_m3']:.6f}"
                for row in appearances
            )
            handle.write(f"{data_name},{volumes}\n")


def index_human_x6_drive_resources(environment, resource_roots):
    result = {}
    for asset_path in environment.container:
        normalized = asset_path.lower()
        if not normalized.endswith("x6.prefab"):
            continue
        for resource_root in resource_roots:
            marker = f"/ships/{resource_root}/"
            if marker not in normalized:
                continue
            ship = normalized.split(marker, 1)[1].split("/", 1)[0]
            for family in ("delaval", "magnetic"):
                if f"/{family}/" not in normalized:
                    continue
                key = (resource_root, ship, family)
                if key in result:
                    raise RuntimeError(
                        f"Duplicate x6 drive resource for {key}: "
                        f"{result[key]} and {asset_path}"
                    )
                result[key] = asset_path
    return result


def measure_human_drive_scales(measure, base_env, dlc_env):
    base_resources = index_human_x6_drive_resources(
        base_env, ("earth", "earth_alt")
    )
    dlc_resources = index_human_x6_drive_resources(
        dlc_env, ("earth_prm", "dlca")
    )
    roots = ("earth", "earth_alt", "earth_prm", "dlca")
    references = {}
    for family in ("delaval", "magnetic"):
        path = base_resources[("earth", "gunship", family)]
        references[family] = measure.measure_drive_resource(
            base_env, path
        )["transverse_bounding_area_m2"]

    result = {}
    for data_name in measure.PREFABS:
        ship = data_name.lower()
        result[data_name] = {"delaval": [], "magnetic": []}
        for appearance_index, resource_root in enumerate(roots):
            environment = base_env if appearance_index < 2 else dlc_env
            resources = base_resources if appearance_index < 2 else dlc_resources
            for family in ("delaval", "magnetic"):
                key = (resource_root, ship, family)
                if key not in resources:
                    raise RuntimeError(f"Missing x6 drive resource for {key}")
                area = measure.measure_drive_resource(
                    environment, resources[key]
                )["transverse_bounding_area_m2"]
                result[data_name][family].append(area / references[family])
    return result


def write_runtime_drive_scale_csv(scales, path):
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(
            "# Generated by scripts/ship-balance/generate_hull_variant_report.py "
            "from measured x6 drive-resource envelopes.\n"
        )
        handle.write(
            "dataName,deLavalAppearanceScales,magneticAppearanceScales\n"
        )
        for data_name in sorted(scales):
            de_laval = "|".join(
                f"{value:.6f}" for value in scales[data_name]["delaval"]
            )
            magnetic = "|".join(
                f"{value:.6f}" for value in scales[data_name]["magnetic"]
            )
            handle.write(f"{data_name},{de_laval},{magnetic}\n")
...
def main():
    ...
    write_runtime_volume_csv(rows, args.runtime_volume_csv.resolve())
    write_runtime_drive_scale_csv(
        human_drive_scales, args.runtime_drive_scale_csv.resolve()
    )
