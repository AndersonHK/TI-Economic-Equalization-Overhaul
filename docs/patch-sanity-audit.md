...
Status: current through Version 0.9.4 and Terra Invicta 1.0.51.
...
|---|---|---|
| `InvestmentPointsPatch` | GDP / $100B | Aligned. The low-income ramp remains; outputMultiplier is 1.10 in 0.9.8, partially compensating recurring national asset upkeep. |
| `NationalSpaceAssetUpkeepPatch` | National MC, raw Boost/month, raw Funding/month | Deducts 0.05/0.10/0.001 monthly IP per corresponding unit after existing penalties and military upkeep, with a zero floor. No federation receipt or faction stockpile taxation. |
| `BoostCapPriorityPatch`, `BoostCapFacilityPatch`, `BoostCapDirectInvestmentPatch`, `BoostCapEconomicChangePatch` | GDP and Education | k=2 national cap, no per-region floor. Preserves existing over-cap production, limits positive increments and direct-investment batches, and refreshes eligibility when economic capacity crosses the current stock. |
...
| Historical-claim scenario registry | Active scenario and DLC entitlement | Aligned. Native flags are preserved; reviewed additions and the three expansion-project families are normalized from active bilateral templates. Dark Skies identifiers are never resolved unless TI has completed DLC validation, the entitlement list contains `DarkSkies`, and the active scenario requires it. Russia-to-Ukraine is explicitly excluded in 2003. Refreshes serialize template enumeration, construct a local set, and atomically publish an immutable snapshot; a failed rebuild retains the last valid snapshot rather than taking down the claim UI. |
...
| `InvestmentTooltipPatch` | UI mirror | Aligned. It renders the native base/available narrative and conditional prose bullets using live army/navy upkeep and MC/Boost/Funding deductions, with repair debt separately identified. The duplicate diagnostic block is removed. |
| `NationalBoostValuePatch`, `NationalBoostTooltipPatch` | UI mirror | Show current (maximum) monthly Boost using the shared economic cap and MC-style wording. Preserve the native trend sprite and 31-day change. The display patch receives structural/IL validation because desktop CLR cannot detour its native Unity UI method; the tooltip patch receives Harmony emission validation. Visual acceptance remains in-game. |
