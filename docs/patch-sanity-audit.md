...
| `EconomyGrowthPatch` | Fixed total GDP gain per IP | Revision planned. Converting a fixed total gain to TI's per-capita getter remains scale-correct, but the isolated exponential GDP/c decay treats wealth as a penalty and cannot let technology loosen the labor/resource soft cap. Replace it with the factor-balance model in `design-directives.md`; retain GDP-linear IP and scale neutrality. |
...
- The metadata and assembly target TI 1.0.47. Verification compiles against the
  installed 1.0.47 assemblies and confirms every guarded gameplay/UI IL anchor.
